...
def allegiance_matrix(M): # M n*t 节点 i和j 属于同一个communiy的个数，除以窗口的数量上
...
def allegiance_matrix_opti(M):
    ...
    probability that node i and node j have been assigned to the same community (Bassett et al., 2014; Mattar et al., 2015).
...
def dumming(labels): # 变成one-hot 编码
...
def single_window_allegiance(t): # 计算在一个窗口内 节点 i和j是否在同一个社区，返回的是一个二值矩阵
    n_nod = len(t)
...
def single_window_allegiance_mean(M): # M : rep * nod 
    n_opt = M.shape[0]
    n_nod = M.shape[1]
    T = np.zeros((n_opt, n_nod, n_nod))

    for i in range(n_opt):
        T[i, :, :] = single_window_allegiance(M[i, :]) # T:
    
    return T.mean(axis=0) # 计算在一个窗口内 节点 i和j的二值矩阵 在多次随机化的中 均值

def all_window_allegiance_mean(M): # M : rep * nod * t
    ...
    return T # T: window-based allegiance matrixs, each matrix is mean over all reptetion # 返回的是 每个窗口 节点 i和j的二值矩阵 在多次随机化的中 均值
